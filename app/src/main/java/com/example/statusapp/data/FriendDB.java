...
import com.example.statusapp.Models.Friend1;
...
public final class FriendDB {
    ...
    private static com.example.statusapp.data.FriendDB instance = null;

    public static com.example.statusapp.data.FriendDB getInstance(Context context) {
        if (instance == null) {
            instance = new com.example.statusapp.data.FriendDB();
    ...
    public long addFriend(Friend1 friend1) {
        SQLiteDatabase db = mDbHelper.getWritableDatabase();
        // Create a new map of values, where column names are the keys
        ContentValues values = new ContentValues();
        values.put(FeedEntry.COLUMN_NAME_ID, friend1.id);
        values.put(FeedEntry.COLUMN_NAME_NAME, friend1.name);
        values.put(FeedEntry.COLUMN_NAME_PHONE, friend1.phone);
        values.put(FeedEntry.COLUMN_NAME_ID_ROOM, friend1.idRoom);
        values.put(FeedEntry.COLUMN_NAME_AVATA, friend1.avata);
    ...
    public void addListFriend(ListFriend listFriend){
        for(Friend1 friend1: listFriend.getListFriend()){
            addFriend(friend1);
        }
    }
...
// you will actually use after this query.
        try {
            ...
            while (cursor.moveToNext()) {
                ...
                friend1.id = cursor.getString(0);
                friend1.name = cursor.getString(1);
                friend1.phone = cursor.getString(2);
                friend1.idRoom = cursor.getString(3);
                friend1.avata = cursor.getString(4);
                listFriend.getListFriend().add(friend1);
            }
            cursor.close();
        }catch (Exception e){
