...
public class GroupDB {
    private static com.example.statusapp.data.GroupDB.GroupDBHelper mDbHelper = null;
    ...
    private static com.example.statusapp.data.GroupDB instance = null;

    public static com.example.statusapp.data.GroupDB getInstance(Context context) {
        if (instance == null) {
            instance = new com.example.statusapp.data.GroupDB();
            mDbHelper = new com.example.statusapp.data.GroupDB.GroupDBHelper(context);
        }
        return instance;
    }
    ...
    public Group getGroup(String id){
        ...
        Cursor cursor = db.rawQuery("select * from " + com.example.statusapp.data.GroupDB.FeedEntry.TABLE_NAME + " where " + FeedEntry.COLUMN_GROUP_ID +" = " + id, null);
    ...
    public ArrayList<Group> getListGroups() {
        ...
        try {
            Cursor cursor = db.rawQuery("select * from " + com.example.statusapp.data.GroupDB.FeedEntry.TABLE_NAME, null);
            while (cursor.moveToNext()) {
                String idGroup = cursor.getString(0);
                String nameGroup = cursor.getString(1);
                String admin = cursor.getString(2);
    ...
    private static final String SQL_CREATE_ENTRIES =
            "CREATE TABLE " + FeedEntry.TABLE_NAME + " (" +
                    ...
                    com.example.statusapp.data.GroupDB.FeedEntry.COLUMN_GROUP_MEMBER + TEXT_TYPE + COMMA_SEP +
                    "PRIMARY KEY (" + com.example.statusapp.data.GroupDB.FeedEntry.COLUMN_GROUP_ID + COMMA_SEP +
                    com.example.statusapp.data.GroupDB.FeedEntry.COLUMN_GROUP_MEMBER + "))";

    private static final String SQL_DELETE_ENTRIES =
            "DROP TABLE IF EXISTS " + com.example.statusapp.data.GroupDB.FeedEntry.TABLE_NAME;
